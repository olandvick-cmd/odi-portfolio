...
export default function LoginPage() {
  ...
  // State for creative mouse tracking interaction
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    setLoading(true);
    const loginToast = toast.loading("Verifying identity coordinates...");
    ...
    try {
      ...
      if (data?.user) {
        toast.success("Access Granted. Welcome back, Admin.", { id: loginToast });
        router.push("/admin");
        router.refresh();
      }
    } catch (err) {
      toast.error("Security handshake failed.", { id: loginToast });
      setLoading(false);
    }
  }
  ...
  return (
    <main className="relative min-h-screen bg-[#030303] flex items-center justify-center px-6 overflow-hidden select-none font-sans">
      ...
      {/* INTERACTIVE BACKGROUND MATRIX LAYER */}
      {/* Dynamic Cyber Pointer Flare */}
      <div 
      ...
      {/* Cyber Grid Vector Array Canvas Lineart */}
      ...
      {/* THE MAIN TERMINAL LOGIC CARD */}
      <div className="relative w-full max-w-md">
        {/* Abstract Corner Aesthetic Brackets */}
        ...
        >
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex space-x-1.5">
              ...
              <span className="w-1.5 h-1.5 rounded-full bg-zinc-700" />
              <span className={`w-1.5 h-1.5 rounded-full ${loading ? 'bg-purple-500 animate-pulse' : 'bg-emerald-500'}`} />
            </div>
          </div>
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-zinc-400 font-mono">
              Provide identity tokens to decrypt space resources.
            </p>
          </div>
